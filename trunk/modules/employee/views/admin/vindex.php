<hgroup id="main-title" class="thin">
		...
		<div class="with-padding">
			...
			<div class="table-header button-height">
				<div class="float-right">
                    <form method="post" action="<?=site_url('employee/admin/search')?>">
					Search&nbsp;<input type="text" name="table_search" id="table_search" value="<?=$this->session->userdata('admin_search')?>" class="input mid-margin-left">
				...
				Show&nbsp;
                <?=form_dropdown('show',config_item('per_page'),$this->session->userdata('admin_paging'),'id="show" class="select blue-gradient glossy" onchange="changeUrl();" ');?>
			</div>
			<table class="table responsive-table" id="sorting-example1">

				<thead>
					<tr>
						<th scope="col" width="10%">User Login</th>
						<th scope="col" class="align-left hide-on-mobile">Nama Lengkap</th>
						<th scope="col" class="align-left hide-on-mobile">Password Login</th>
				...
				<tbody>
					...
					<tr>
						<td><?=$row['ID'];?></td>
						<td><?=$row['Name'];?></td>
						<td><?=$row['PasswordWeb'];?></td>
						<td class="low-padding align-center">
                            <a href="<?=site_url('employee/admin/edit/'.$row['ID'])?>" class="button white-gradient glossy">Update</a>
						</td>
					</tr>
                    <?php endforeach;?>
			...
			<form method="post" action="" class="table-footer button-height large-margin-bottom">
                ...
                -->
                Show <?=form_dropdown('show2',config_item('per_page'),$this->session->userdata('admin_paging'),'id="show2" class="select blue-gradient glossy" onchange="changeUrl2();" ');?>
			</form>
     ...
     <script>
        function changeUrl() {
            ...
            document.location.href = '<?=site_url("employee/admin/paging")?>/' + redirect;
        }
