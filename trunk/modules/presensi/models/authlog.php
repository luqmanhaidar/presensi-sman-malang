...
class Authlog extends CI_Model
{
	...
	function getDay($date_start='',$date_finish=''){
        ...
        $this->db->order_by('COUNT(TransactionTime)','DESC');
    ...
    function getMonthRecords($offset='',$paging='',$month='',$year='',$group=''){
		...
		$this->db->select('NGAC_AUTHLOG.UserID,NGAC_USERINFO.Name,NGAC_USERINFO.Description,NGAC_USERINFO.Department,NGAC_GROUP.Name as GroupName,MIN(NGAC_AUTHLOG.TransactionTime) as TransactionTime,MAX(NGAC_AUTHLOG.TransactionTime) as TransactionTimeMax,COUNT(DISTINCT(CONVERT(VARCHAR(10),TransactionTime, 105))) as Total ');
        $this->db->join('NGAC_USERINFO','NGAC_USERINFO.ID=NGAC_AUTHLOG.UserID');
		$this->db->join('NGAC_GROUP','NGAC_GROUP.ID=NGAC_USERINFO.GroupID');
        $this->db->where_not_in('NGAC_USERINFO.Privilege',1);
        $this->db->where_not_in('datename(dw,TransactionTime)','Sunday');
